...
io.on("connection", (socket) => {
  ...
  socket.on("user_connected", (userId: string) => {
    userSocketMap[userId] = socket.id;
    console.log(`User ${userId} explicitly connected with socket ${socket.id}`);
  });
  ...
  socket.on("typing", ({ toUserId }: { toUserId: string }) => {
    ...
    if (targetSocketId) {
      io.to(targetSocketId).emit("typing", { fromUserId: socket.handshake.query.userId });
    }
  });
  ...
  socket.on("stop_typing", ({ toUserId }: { toUserId: string }) => {
    const targetSocketId = getReceiverSocketId(toUserId);
    if (targetSocketId) {
      io.to(targetSocketId).emit("stop_typing", { fromUserId: socket.handshake.query.userId });
    }
  });

  // Group chat typing events
  socket.on("group_typing", ({ groupId }: { groupId: string }) => {
    socket.to(groupId).emit("group_typing", { fromUserId: socket.handshake.query.userId, groupId });
  });

  socket.on("group_stop_typing", ({ groupId }: { groupId: string }) => {
    socket.to(groupId).emit("group_stop_typing", { fromUserId: socket.handshake.query.userId, groupId });
  });
